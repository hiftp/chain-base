odoo.define('search.abc.abnormal.transaction', function (require) {
    "use strict";
    ...
    let InterfaceAbcAbnormalTransaction = Widget.extend({
        events: {
            ...
            "click #search": function (env) {
                ...
                let payment_id = self.$el.find('#payment_id').val();
                if (account_id == '0') {
                ...
                new Model('interface.abc.abnormal.transaction').call('search_abc_transaction', [account_id, currency_id, payment_id])
                    .then(function (res) {
                        self.$el.find('#search_result').html(QWeb.render("InterfaceAbcAbnormalTransactionResult", {
                            widget: self,
                            data: res,
                        }));
        ...
        start: function () {
            let self = this;
            self.$el.html(QWeb.render("InterfaceAbcAbnormalTransaction", {widget: self}));
    ...
    core.action_registry.add('find_abc_abnormal_transaction', InterfaceAbcAbnormalTransaction);
